...
public class TraderExclusiveInventory extends TraderInventory {

    public TraderExclusiveInventory(Player p) {
        super(ShopCategory.EXCLUSIVE, p);
        CorePlayer cp = CoreSystem.getInstance().getCorePlayer(p);

        for (ShopItem item : ShopItem.values()) {
            if (item.getCategory().equals(ShopCategory.EXCLUSIVE)) {
